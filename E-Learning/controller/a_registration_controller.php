 ...
 function test_input($data) {
  ...
  if (empty($_POST["contactno"])) {
    ...
    if (empty($_POST["username"])) {
      $_SESSION['usernameErra']= "Username is required";
      $flag=0;
    }
    else {
     $username = test_input($_POST["username"]);
      ...
      if (!preg_match("/^[a-zA-Z-. ]*$/",$username)) {
         $_SESSION['usernameErra']= "Only letters and white space allowed";
         $flag=0;
       }
       else {
         if(strlen($username)<2)
         {
            $_SESSION['usernameErra']= "Name have to contains at least two character ";
            $flag=0;
         }
...
if($flag==1)
{
  ...
  if (addInstructor($data)) {
	...
	header('Location: ../a_registration_view.php');
}
